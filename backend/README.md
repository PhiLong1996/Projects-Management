...
This is the backend half of the project — the frontend lives in the
sibling `../frontend/` folder with its own README/Dockerfile/compose. All
commands below assume you're `cd`'d into this `backend/` folder. See the
repo root `README.md` for how the two run together.
...
| `CORS_ORIGINS` | `http://localhost:3000,http://localhost:8080` | Comma-separated origins allowed to call this API from a browser — needed so `../frontend/` (a sibling app, running on its own origin) can reach it. Not used by the WebSocket endpoint (browsers don't apply CORS to WS handshakes). |
...
email delivery (§7), WebSocket + Redis realtime notification delivery (§10),
and a frontend UI (see the repo root README) have since been added, the rest
remain out of scope: PDF export, Celery/Elasticsearch, RabbitMQ (a message
broker for durable work queues wasn't the right fit for realtime
notification fan-out — see §10's explanation of why Redis Pub/Sub was used
there instead), and the optional bonus features (AI assistant, OAuth
logins, Kanban/Gantt views, etc.).

## 15. Frontend

This repo's frontend is the sibling `../frontend/` folder (a separate
Next.js app with its own `README.md`, `Dockerfile`, and `docker-compose.yml`
— deliberately kept independent of this backend so each can be run,
debugged, and deployed on its own). It's a real client of this API, not a
design mockup: login, dashboard, projects/sprints, a task board, task
detail, realtime notifications, and admin user management, all wired to
the endpoints documented above. See the repo root `README.md` for how the
two run together, and `../frontend/README.md` for frontend-specific setup
and the backend gaps that shaped a couple of its choices (no endpoint to
list a project's members; no single-task `GET`).

Because it runs on a different origin than this API during local dev,
`CORS_ORIGINS` (§6) has to list it — the default already includes
`http://localhost:3000`.
